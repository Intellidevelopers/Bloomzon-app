  ...
  if (returns.length === 0) {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
        <Text>No returns available.</Text>
      </View>
    );
  }
    ...
    <ScrollView showsVerticalScrollIndicator={false}>
      {returns.map((item, index) => (
        <Pressable
          ...
          onPress={() =>
            navigation.navigate('ReturnDetails', { returnDetails: item })
          }
        >
          ...
          <View style={styles.returnContent}>
            ...
            <View style={styles.returnDetails}>
              ...
              <Text style={styles.sku}>
                SKU: <Text style={{ color: '#666', fontWeight: '400' }}>{item.sku}</Text>
              </Text>
              <Text style={styles.reason}>
                Reason: <Text style={{ color: '#666', fontWeight: '400' }}>{item.reason}</Text>
              </Text>
            </View>
          </View>
        </Pressable>
